...
\newpage
        \item Plotting  the natural $\log$ ($\ln$) of the training error versus the polynomial dimension $k = 1, . . . , 18$:
            \begin{figure}[h]
            \centering
            \includegraphics[scale=0.5]{outputs/q2/q2b}
...
\subsection{Filtered Boston housing and kernels}
    ...
    \begin{enumerate}
        ...
        Here are the plots of training and test error:\\
        \includegraphics[scale=0.5]{outputs/q4/q4_train.png}\\
        \includegraphics[scale=0.5]{outputs/q4/q4_test.png}\\
    \end{enumerate}
\newpage
...
\\
Here is our contour plot for mse over our parameter grid:\\

\includegraphics[scale = 0.5]{outputs/q5/contour_plot.png}\\

\item[5.]

Here is our final regression report over all oour models, run 20 times each.\\

\item The mean squared error for each fitted curve:
\begin{center}
\begin{tabular}{c|c|c|c|c}%
\textbd{model}&\textbf{train mse}&\textbf{train std}&\textbf{test mse}&\textbf{test std}% specify table head
\csvreader[head to column names]{outputs/q5/regression_report.csv}{}% use head of csv as column names
{\\\hline\csvcoli&\csvcolii&\csvcoliii&\csvcoliv&\csvcolv}% specify your coloumns here
\end{tabular}
\end{center}
...
\item[9.] \begin{enumerate}
    ...
    Hence, adding c to our kernel acts like a regulariser in the limit.\\ 
\end{enumerate}
